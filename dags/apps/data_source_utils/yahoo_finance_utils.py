...
class YahooFinanceScraper(BeautifulSoupScraper):
	...
	def test_get_data(self, url: str):
		return super().request_webpage(url="https://finance.yahoo.com/quote/GOOGL/profile/")
	...
	def _get_data(self, url: str) -> Dict:
		...
		data_record = {}
		extract_config = self.config.DIM_DATA_EXTRACT_CONFIG if url.endswith("profile/") else self.config.DAILY_EXTRACT_CONFIG
			...
			try:
				...
				if field_config.get("is_data_value"):
					...
					data_value = super().get_element_text_value(
						...
						sibling_html_element_value_tag=field_config.get("sibling_html_element_value_tag"),
					...
					raise ValueError(f"Unspecified config value type for config target key: {field_config.get('target_field')}")
				print(f"{field_config.get('target_field')}: {data_value}")
				data_record[field_config.get("target_field")] = data_value
			except Exception as e:
				raise Exception(f"Get value for {field_config.get('target_field')} for {url} failed with Exception: {e}") from e
		return data_record
	...
	def get_daily_data(self) -> List:
		...
		for company_symbol in self.config.SP_500_SYMBOLS_CONFIG:
			daily_data.append(self._get_data(url=f"https://finance.yahoo.com/quote/{company_symbol}/"))
		return daily_data
	...
	def get_dim_data(self) -> List:
		...
		dim_data = []
		for company_symbol in self.config.SP_500_SYMBOLS_CONFIG:
			dim_data.append(self._get_data(url=f"https://finance.yahoo.com/quote/{company_symbol}/profile/"))
		return dim_data
